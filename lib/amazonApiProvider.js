...
var throttledRequest =  limit(function(id, callback) {
...
var itemScrape = function(id, callback) {
    ...
    var proxy = secrets.proxy[Math.floor(Math.random()*(2))];
    request.get({
        url: amazonUrl,
        headers: {
            'User-Agent' : ''
        ...
        if (error) return callback(error);
            ...
            console.log("Bad response status code for ASIN: " + id + ": " + response.statusCode);
            callback(null, {});
            return;
        }
        x(body || '', {
...
exports.getItemsFromAmazon = function(asins, callback) {
    ...
    _.each(asinsLists, function(asinsBatch) {
        callArray.push(function(asyncCallback) {
            throttledRequest(asinsBatch.join(','), function(data) {
...
exports.itemLookup = throttledRequest;
