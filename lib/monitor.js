...
var processItem = function(item, userSettings, listing) {
    ...
    if ((priceToCompare && (parseFloat(priceToCompare) !== parseFloat(listingPrice))) || item.inStock) {
        if (!(listing.lastRevisionAction == 'Revision')) {
            result.price = priceToCompare;
            result.quantity = userSettings.itemQuantity;
            result.action = 'Revision';
        }
    }
    if (!item.inStock && !(listing.lastRevisionAction == 'outOfStock')) {
        result.quantity = '0';
    ...
    User.find({}, function (err, users) {
        ...
        _.each(activeUsers, function(user) {
            Listing.find({$and: [{ user_id: user.id}, { status: 'LISTED'}]}, function(err, listings) {
                ...
                amazonProvider.scrapeItemsFromAmazon(asins, function(items) {
                    if (_.isEmpty(items)) {
                        return;
                    }
