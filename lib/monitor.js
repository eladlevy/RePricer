...
var processItem = function(item, userSettings, listing) {
    ...
    if (calculatedPrice && (parseFloat(calculatedPrice) !== parseFloat(listingPrice))) {
    ...
    if ((availability.toLowerCase().indexOf('usually ships in') == -1)|| item.Offers.Offer[0].OfferListing.IsEligibleForPrime == '0') {
        result.quantity = '0';
                ...
                amazonProvider.getItemsFromAmazon(asins, function(items) {
                    ...
                    ebayProvider.reviseMultipleInventoryStatus(ebayToken, itemsSummary, function(ebayErrors) {
                        ...
                        _.each(itemsSummary, function(revisedItem) {
                            ...
                            if (revisedListing.lastRevisionAction == 'outOfStock' && revisedItem.action == 'outOfStock') {
                                return;
                            }
